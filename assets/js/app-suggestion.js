function suggestion() {
    ...
    if (resDeperdition > 12000 || resDeperdition < 200) {
        ...
        if ($("#transparent").prop("checked")) {
            $("#suggestionContent").append('<div id="suggestionTransparentConsole" class="border-secondary card"><p>Pour le dimensionnement on part de la température "critique" (extrême) donc on ne considère que les cas d\'usages critiques des poêles.</p>'
            ...
            $.each(this.dalyPower, function() {
                ...
                if (this.use == "critical") {
                    ...
                    if (diffPowerDeperditionPercentAbs < settings.pdmSuggestion.percentPowerSuper) {
                        ...
                        bestClass = 2;
                        bestClass2++;
                    } else if (diffPowerDeperditionPercentAbs < settings.pdmSuggestion.percentPowerCool) {
                        bestClass = 1;
                        bestClass1++;
        ...
        // Remplissage du tableau pour être transparent
        if ($("#transparent").prop("checked")) {
            $.each(pdmSuggestions, function() {
                $('#suggestionTab > tbody:last-child').append(
                    '<tr class="'+this.trClass+'">'
                        ...
                        +'<td class="text-center">'+this.woodLoad+'</td>'
